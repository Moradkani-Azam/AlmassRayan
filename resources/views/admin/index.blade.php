                          ...
                          <table class="min-w-full divide-y divide-gray-200 w-full">
                              ...
                              @foreach ($orders as $order)
                                  <tr>
                                      ...
                                      <td class="hidden lg:table-cell px-6 py-4 whitespace-wrap text-sm text-gray-900">
                                        {{ $order->user->name }}
                                      </td>
                                      <td class="hidden lg:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                      ...
                                      <td class="px-6 py-4 whitespace-wrap text-sm font-medium">
                                          <a href="{{ route('admin.orders.single', $order->id) }}" class="text-blue-600 hover:text-blue-900 mb-2 mr-2">{{ __('View') }}</a>
                                          @if($order->status == 1)
                                          <form class="inline-block" action="{{ route('orders.changeStatus', $order->id) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                                              <input type="hidden" name="_method" value="PUT">
                                              <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                              <input type="hidden" name="complete" value="2">
                                              <input type="submit" class="font-bold text-green-600 hover:text-green-900 mb-2 mr-2" value="Complete">
                                          </form>
                                          @endif
                                      </td>
                                  </tr>
